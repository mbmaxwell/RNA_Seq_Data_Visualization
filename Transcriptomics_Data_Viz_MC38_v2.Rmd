...
volcano_plot <- ggplot(data = volcano_df,
                     aes(x = Log2fc,
                         y = -log10(adj_p_value))) + 
...
volcano_plot
```
For presentations, it can be helpful to highlight genes of interest on our volcano plot such as the three upregulated & downregulated genes I've highlighted on this volcano plot. **For context, the two downregulated DEGs I highlighted are immunosuppressive and three upregulated DEGs are pro-inflammatory which could all contribute to our observation of anti-tumor immunity in sgArid1a MC38 tumors compared to sgScramble MC38 tumors and are thus of value to highlight on the volcano plot.**
...
```
**Heatmaps are useful for visualizing the relative relationships between gene expression values where rows are individual genes and columns are samples.** You can also communicate how many genes are upregulated versus downregulated in relative terms with heatmaps. Here, I've highlighted the same six genes I highlighted in the volcano plot.
